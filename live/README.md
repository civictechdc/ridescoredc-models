# live/ — what deploys to staging & prod

This folder is the **source of truth for the database schema, seed data, and tile
functions that reach the shared databases.** Model developers experiment in their
own WIP areas (`../bna/`, `../lts/`, notebooks); when a change is ready to ship,
it lands here as a migration and is applied to staging first, then prod.

Ownership boundary: `live/` owns the **model-produced** tables (`ridescoredc`,
`crashes_dc`) and the **tile functions**. The **survey tables** hold runtime user
data written by `ridescoredc-website`. Their schema is **not** yet managed by a
yoyo migration — the DDL lives here only as the work-in-progress
`schema/wip-patch.sql` (moved out of the website's old boot-time patch), pending
being folded into a real migration. Nothing applies it automatically yet.
